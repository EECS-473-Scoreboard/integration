...
int main(void) {
    ...
    while (1) {
        ...
        {
            button_t button = poll_button();
            if (current_scr == GAME_SCR && button != BUTTON_NONE) {
                // only send events generated by physical buttons when at game
                lv_event_send(game_screen, SC_EVENT_BUTTON,
        ...
        // read_touch(&drv, &data);
        // fb[data.point.y * LCD_RENDER_WIDTH + data.point.x] = 0xFF;
        HAL_Delay(5);
        /* USER CODE END WHILE */
